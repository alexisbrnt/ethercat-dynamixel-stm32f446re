...
static void lan9252_write_32(uint16_t address, uint32_t val) {
	...
	spi_select(lan9252);
	/* Write data */
	spi_write(lan9252, data, sizeof(data));
	/* Un-select device. */
...
uint32_t lan9252_read_32(uint32_t address) {
	...
	/* Read data */
	spi_write(lan9252, data, sizeof(data));
	spi_read(lan9252, result, sizeof(result));
	/* Un-select device. */
	spi_unselect(lan9252);


	ret = (result[3] << 24) | (result[2] << 16) | (result[1] << 8)
			| (result[0]);
...
/*static*/void ESC_read_pram(uint16_t address, void *buf, uint16_t len) {
	...
	while (len > 0) {
		...
		/* Always read 4 byte */
		spi_read(lan9252, (temp_buf + byte_offset), sizeof(uint32_t));
...
/* static */void ESC_write_pram(uint16_t address, void *buf, uint16_t len) {
	...
	while (len > 0) {
		...
		/* Always write 4 byte */
		spi_write(lan9252, (void*) &value, sizeof(value));
